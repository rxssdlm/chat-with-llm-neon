...
Módulo CRM construido sobre `chat-with-llm` (FastAPI + GROQ/Agno + PostgreSQL) como
proyecto final del diplomado. Implementa un agente conversacional con **tool calling**,
**memoria persistente por usuario**, **seguridad anti prompt-injection**, **RBAC** y
...
Además de las ya usadas por el proyecto (`GROQ_API_KEY`, `DEFAULT_MODEL`, etc.),
se requiere `DATABASE_URL` apuntando a PostgreSQL (usado tanto por SQLAlchemy/Alembic
